...
__all__ = ["SingleIdTokenIndexer", "TokenCharactersIndexer"]
...
'''
class Tokenizer:
    def __init__(self, split="-/,)&(?", prefix="\".'", suffix="\"'.:;!"):
        self.split = split
        self.prefix = prefix + split
        self.suffix = suffix + split

    def _over_split_token(self, token, start):
        tokens = []

        if len(token) == 1 or token.startswith("http") or token == "'s":
            tokens.append(Token(token, character_offset=start))
            return tokens

        if len(set(token[0:3])) == 1 or len(set(token[-3:])) == 1:
            tokens.append(Token(token, character_offset=start))
            return tokens

        if len(token) > 2 and token[-2:] == "'s":
            tokens += self._over_split_token(token[0:-2], start)
            tokens += self._over_split_token(token[-2:], start + len(token) - 2)
            return tokens

        if len(token) > 1 and token[-1] in self.suffix:
            tokens += self._over_split_token(token[0:-1], start)
            tokens += self._over_split_token(token[-1], start + len(token) - 1)
            return tokens

        if len(token) > 1 and token[0] in self.prefix:
            tokens += self._over_split_token(token[0], start)
            tokens += self._over_split_token(token[1:], start + 1)
            return tokens

        where_is_split = -1
        for s in self.split:
            where_is_split = token.find(s)
            if where_is_split > 0:
                break

        if where_is_split > 0:
            tokens += self._over_split_token(token[0:where_is_split], start)
            tokens += self._over_split_token(token[where_is_split:], start + where_is_split)
            return tokens
    ...
    def tokenize(self, text, start=0):
        tokens, start_with_text = [], 0
        for token in text.strip().split():
            start_with_text = text.find(token, start_with_text)
            tokens += self._over_split_token(token, start + start_with_text)
            start_with_text += len(token) + 1
        return tokens
'''
...
Represent tokens as single intergers'''
class SingleIdTokenIndexer:
    def __init__(self, lowercase_tokens=True, normalize_digits=False):
        self._key = "tokens"
    ...
    def tokens_to_indices(self, tokens, vocabulary, index_name):
        ...
        for token in tokens:
            ...
            if self.lowercase_tokens: text = text.lower()
            if self.normalize_digits: text = re.sub(r"[0-9]", "0", text)
            indices.append(vocabulary.get_item_index(text, self._key))
...
class PosIndexer:
    ...
    def count_vocab_items(self, token, counter):
        pos = token.pos.lower()
        counter[self._key][pos] += 1

    def tokens_to_indices(self, tokens, vocabulary, index_name):
        indices = []
        for token in tokens:
            pos = token.pos.lower()
            indices.append(vocabulary.get_item_index(pos, self._key))
        return {index_name: indices}
...
class TokenCharactersIndexer:
    def __init__(self):
        self._key = "token_characters"
        self._min_padding_length = 3    # If using CnnEncoder to build character-level representations,
                                        # this value is set to the maximum value of ngram_filter_sizes


    def count_vocab_items(self, token, counter):
        for c in list(token.text):
            counter[self._key][c] += 1
    ...
    def tokens_to_indices(self, tokens, vocabulary, index_name):
        ...
        for token in tokens:
            token_indices = []
            for c in list(token.text):
                index = vocabulary.get_item_index(c, self._key)
    ...
    def get_padding_lengths(self, token):
        return {"num_%s" % self._key: max(len(token), self._min_padding_length)}
    ...
    '''
    def pad_token_sequence(self, tokens, desired_num_tokens, padding_lengths):
        padded_tokens = pad_sequence_to_length(tokens[self._key], desired_length=desired_num_tokens[self._key],
                                               default_value=self.get_padding_token)
        desired_token_length = padding_lengths["num_%s" % self._key]
        longest_token_length = max([len(t) for t in tokens[self._key]])
        padding_value = 0
        ...
        return {self._key: [list(token[:desired_token_length]) for token in padded_tokens]}



'''Reference url: https://github.com/allenai/allennlp/blob/master/allennlp/data/token_indexers/elmo_indexer.py
